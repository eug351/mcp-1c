...
Шаги:
1. Используй инструмент code_read с action=object_structure чтобы посмотреть структуру объекта (object_type: %s, object_name: %s)
2. Используй инструмент code_search с action=text чтобы найти код модулей этого объекта
...
4. Если нужна справка по встроенным функциям -- используй инструмент code_search с action=syntax_help
...
Шаги:
1. Используй инструмент code_read с action=object_structure чтобы посмотреть структуру документа (object_type: Document, object_name: %s) -- реквизиты и табличные части
2. Используй инструмент code_search с action=text чтобы найти текущий код модуля объекта
3. Используй инструмент code_read с action=metadata_tree чтобы увидеть доступные регистры накопления, сведений и бухгалтерии
4. Для каждого регистра, в который должен записывать документ, используй code_read с action=object_structure чтобы узнать его измерения, ресурсы и реквизиты
...
6. Если нужна справка по синтаксису -- используй инструмент code_search с action=syntax_help`, documentName, documentName),
	)
}
...
Шаги:
1. Используй инструмент code_execute с action=query чтобы выполнить запрос и оценить объём возвращаемых данных
2. Используй инструмент code_read с action=metadata_tree чтобы увидеть доступные объекты метаданных
3. При необходимости используй code_read с action=object_structure для проверки структуры таблиц, участвующих в запросе
...
Шаги:
1. Используй инструмент code_read с action=metadata_tree чтобы получить полное дерево метаданных конфигурации
...
2. Если ошибка указывает на конкретный объект метаданных -- используй code_read с action=object_structure для просмотра его структуры
3. Если ошибка связана с кодом модуля -- используй code_search с action=text для поиска исходного кода
4. Если ошибка связана с запросом -- используй code_execute с action=query для проверки запроса
5. Если нужна справка по функциям -- используй code_search с action=syntax_help
6. Объясни:
   - Причину ошибки
...
Шаги:
1. Используй инструмент code_read с action=object_structure чтобы посмотреть структуру объекта (object_type: %s, object_name: %s)
2. Используй инструмент code_search с action=text чтобы найти код модулей объекта
3. Проанализируй код на:
...
Шаги:
1. Используй инструмент code_read с action=metadata_tree чтобы увидеть доступные объекты метаданных (справочники, документы, регистры)
2. Определи источники данных для отчёта и используй code_read с action=object_structure для каждого из них, чтобы узнать структуру полей
3. Используй code_execute с action=query чтобы проверить пробный запрос и убедиться что данные доступны
4. Если нужна справка по синтаксису запросов или функций -- используй code_search с action=syntax_help
5. Напиши:
...
Шаги:
1. Используй инструмент code_read с action=object_structure чтобы получить полную структуру объекта (object_type: %s, object_name: %s) -- реквизиты, табличные части, измерения, ресурсы
2. Используй инструмент code_search с action=text чтобы найти код модулей объекта
3. Используй инструмент code_read с action=metadata_tree чтобы увидеть другие объекты конфигурации и понять связи
4. Объясни:
